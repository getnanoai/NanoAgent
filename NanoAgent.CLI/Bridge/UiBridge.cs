    ...
    public void ShowToolResults(ToolExecutionBatchResult toolExecutionResult)
    {
        ...
        EnqueueForActiveOperation(state =>
        {
            ...
            for (int index = 0; index < messages.Count; index++)
            {
                ...
                resultMsg.CompactToolOutputText = index < compactMessages.Count
                    ? NormalizeTabs(compactMessages[index])
                    : message;
                resultMsg.FullToolOutputText = index < fullMessages.Count
                    ? NormalizeTabs(fullMessages[index])
                    : message;
