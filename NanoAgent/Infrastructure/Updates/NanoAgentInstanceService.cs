...
/// </summary>
internal sealed class NanoAgentInstanceService : INanoAgentInstanceService
{
    // Process names that identify a running NanoAgent CLI binary: the archive
    // executable (NanoAgent.CLI) and the installed command name (nanoagent).
    private static readonly HashSet<string> NanoAgentProcessNames = new(StringComparer.OrdinalIgnoreCase)
    {
        "nanoagent",
        "nanoagent.cli"
    };
    ...
    public NanoAgentInstanceService(IProcessRunner processRunner)
    {
        _processRunner = processRunner;
    }

    public async Task<IReadOnlyList<RunningNanoAgentInstance>> GetOtherRunningInstancesAsync(
        CancellationToken cancellationToken)
    {
        int currentProcessId = Environment.ProcessId;
        List<RunningNanoAgentInstance> instances = [];
        ...
        try
        {
            ...
            {
                RunningNanoAgentInstance? instance = ParseLine(line, currentProcessId);
    ...
    public async Task TerminateAsync(RunningNanoAgentInstance instance, CancellationToken cancellationToken)
    {
    ...
    internal static RunningNanoAgentInstance? ParseLine(string line, int currentProcessId)
    {
    ...
    internal static RunningNanoAgentInstance? ParseWindowsLine(string line, int currentProcessId)
    {
        List<string> fields = [];
        ...
        if (processId == currentProcessId || !MatchesNanoAgentName(name))
        {
            return null;
        }

        return new RunningNanoAgentInstance(processId, name);
    }

    internal static RunningNanoAgentInstance? ParsePosixLine(string line, int currentProcessId)
    {
        line = line.Trim();
        if (line.Length == 0)
        {
    ...
    internal static bool MatchesNanoAgentName(string name)
    {
        ...
        return NanoAgentProcessNames.Contains(normalized);
    }
}
