...
test("matchesNanoAgent recognizes NanoAgent binaries", () => {
  assert.equal(instances.matchesNanoAgent("nanoagent"), true);
  assert.equal(instances.matchesNanoAgent("NanoAgent.CLI"), true);
  assert.equal(instances.matchesNanoAgent("nanoagent.exe"), true);
  assert.equal(instances.matchesNanoAgent("NANOAGENT.EXE"), true);
  assert.equal(instances.matchesNanoAgent("dotnet"), false);
  assert.equal(instances.matchesNanoAgent("node"), false);
  assert.equal(instances.matchesNanoAgent(""), false);
});

test("parseWindowsTaskList parses tasklist CSV and excludes current pid", () => {
  const text = '"nanoagent.exe","1234","Services","0","12,345 K"\r\n"dotnet.exe","5678","Console","1","1 K"\r\n';
  const result = instances.parseWindowsTaskList(text, 5678);
  assert.deepEqual(result, [{ pid: 1234, name: "nanoagent.exe" }]);
});

test("parsePosixPs parses ps output and skips the header", () => {
  const text = "  PID COMM\n 1234 NanoAgent.CLI\n 5678 dotnet\n";
  const result = instances.parsePosixPs(text, 5678);
  assert.deepEqual(result, [{ pid: 1234, name: "NanoAgent.CLI" }]);
});

test("findOtherInstances uses tasklist on Windows", async () => {
  const text = '"nanoagent.exe","1234","Services","0","1 K"\r\n"dotnet.exe","2","Console","1","1 K"\r\n';
  const mock = (cmd) => {
  ...
  assert.deepEqual(found, [{ pid: 1234, name: "nanoagent.exe" }]);
});
...
test("findOtherInstances uses ps on POSIX", async () => {
  ...
  const found = await withOverrides({ platform: "linux", spawnSync: mock }, () =>
    instances.findOtherInstances());
  assert.deepEqual(found, [{ pid: 1234, name: "NanoAgent.CLI" }]);
});
...
test("terminateInstance runs taskkill on Windows", async () => {
  ...
  await withOverrides({ platform: "win32", spawnSync: mock }, () =>
    instances.terminateInstance({ pid: 1234, name: "nanoagent.exe" }));
...
test("terminateInstance runs kill on POSIX", async () => {
  ...
  await withOverrides({ platform: "linux", spawnSync: mock }, () =>
    instances.terminateInstance({ pid: 1234, name: "NanoAgent.CLI" }));
...
test("terminateOtherInstances terminates when the prompt is approved", async () => {
  ...
  assert.deepEqual(terminated, [{ pid: 1234, name: "NanoAgent.CLI" }]);
