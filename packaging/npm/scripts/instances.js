...
const NANOAGENT_PROCESS_NAMES = new Set(["nanoagent", "nanoagent.cli"]);
...
function matchesNanoAgent(name) {
  return NANOAGENT_PROCESS_NAMES.has(normalizeName(name));
}
...
// Parses `tasklist /NH /FO CSV` output into NanoAgent instances, excluding the
// current process id.
...
// Parses `ps -eo pid,comm` output into NanoAgent instances, excluding the
...
// Returns running NanoAgent CLI instances other than the current node process.
...
async function promptToTerminate(instances, selectLoader = loadSelect) {
  ...
  return await select({
    message: `${instances.length} other NanoAgent instance(s) are running (${list}). Terminate them and update?`,
    choices: [
      {
...
// Orchestrates the running-instance check: finds other NanoAgent sessions,
...
async function terminateOtherInstances(options = {}) {
  ...
  log(`Terminating ${instances.length} other NanoAgent instance(s)...`);
